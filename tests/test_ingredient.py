...
class TestIngredient:
    ...
    @allure.title('Проверяем конструктор ингредиента __init__()')
    def test_init_ingredient(self):
        sauce = Ingredient(SAUCE_TYPE, SAUCE_NAME, SAUCE_PRICE)
        assert sauce.type == SAUCE_TYPE and sauce.name == SAUCE_NAME and sauce.price == SAUCE_PRICE

    @allure.title('Проверяем конструктор __init__() с пустым и незаданным типом ингредиента')
    ...
    @allure.title('Проверяем конструктор __init__() с пустым и незаданным названием ингредиента')
    @pytest.mark.parametrize('name', ['', None])
    def test_init_ingredient_name(self, name):
        sauce = Ingredient(SAUCE_TYPE, name, SAUCE_PRICE)
        assert sauce.name == name

    @allure.title('Проверяем конструктор __init__() с нулевой и незаданной стоимостью ингредиента')
